...
class AppSettingsSharedPreferences {
  ...
  Future<void> setDateOfBirth(String date) async {
    await _sharedPreferences.setString(KeyConstants.dateOfBirth, date);
  }

  Future<void> setGender(String gender) async {
    await _sharedPreferences.setString(KeyConstants.gender, gender);
  }

  Future<void> setPhone(String phoneNumber) async {
    await _sharedPreferences.setString(KeyConstants.userPhone, phoneNumber);
  }

  Future<void> setUserName(String userName) async {
    await _sharedPreferences.setString(KeyConstants.userName, userName);
  }

  Future<void> saveUserInfo(User user) async {
    await _sharedPreferences.setInt(KeyConstants.userId, user.id);
    await _sharedPreferences.setString(KeyConstants.userType, user.type);
    await _sharedPreferences.setString(KeyConstants.userName, user.name);
    await _sharedPreferences.setString(KeyConstants.userEmail, user.email);
    await _sharedPreferences.setString(KeyConstants.userAvatar, user.avatar);
    await _sharedPreferences.setString(
      KeyConstants.userAvatarOriginal,
      user.avatarOriginal,
    );
    await _sharedPreferences.setString(KeyConstants.userPhone, user.phone);
  }
  ...
  String get dateOfBirth =>
      _sharedPreferences.getString(KeyConstants.dateOfBirth).onNull();

  String get gender =>
      _sharedPreferences.getString(KeyConstants.gender).onNull();
